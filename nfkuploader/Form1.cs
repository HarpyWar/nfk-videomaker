...
namespace nfkuploader
{
    public partial class Form1 : Form
    {
        public Form1()
        {
            ...
            // get demoFile from url (it was downloaded by nfkscheduler)
            ...
            for (int i = 0; i < demo.players.Length; i++)
            {
                ...
                var pattern = demo.local_videos[i].FileName.Replace(".mp4", "") + "_";// 1234_1.mp4 -> 1234_1_
                ...
                var newVideoFile = demo.local_videos[i].FileName.Replace(".mp4", "_" + demo.local_videos[i].YoutubeId + ".mp4");
            ...
            if (APIClient.SetVideo(videoString, demo.local_appid))
            {
        ...
        {
            foreach (var f in Directory.GetFiles(Config.Data.TempDir, "*.mp4"))
            {
                if (f.StartsWith(startsWith))
                {
                    string youtubeId = f.Substring(startsWith.Length, f.Length - ".mp4".Length);
